...
func (r *bookingRepo) GetBookingContext(ctx context.Context, bookingID int64) (*param.BookingContextParam, error) {
	row := r.DB.QueryRowContext(ctx, `
		...
		JOIN service_slots ss ON ss.service_slot_id = sso.service_slot_id
		JOIN users cu ON cu.user_id = b.user_id
		JOIN users ou ON ou.user_id = bp.owner_user_id
		LEFT JOIN staff st ON st.staff_id = ss.staff_id
	...
	_, err := tx.ExecContext(ctx, `
		...
		WHERE booking_id <> $2
			...
			AND slot_option_id IN (
				SELECT slot_option_id FROM service_slot_options
...
func (r *bookingRepo) SlotOptionIsAvailable(ctx context.Context, slotOptionID int64) (bool, error) {
	...
	err := r.DB.QueryRowContext(ctx, `
		SELECT EXISTS(
				...
				JOIN service_slots ss ON ss.service_slot_id = ssp.service_slot_id
...
func (r *bookingRepo) InsertBooking(ctx context.Context, userID int64, slotOptionID int64, description *string) (*param.BookingParam, error) {
	...
	err := r.DB.QueryRowContext(ctx, `
		INSERT INTO bookings (user_id, slot_option_id, booking_group_id, status, booking_type, description)
		VALUES ($1, $2, nextval('booking_group_seq'), 'pending', 'online', $3)
...
func (r *bookingRepo) InsertWalkInBooking(ctx context.Context, userID int64, slotOptionID int64) (*param.BookingParam, error) {
	var b param.BookingParam
	err := r.DB.QueryRowContext(ctx, `
		INSERT INTO bookings (user_id, slot_option_id, booking_group_id, status, booking_type, decided_by, decided_at)
		VALUES ($1, $2, nextval('booking_group_seq'), 'accepted', 'walk_in', $1, NOW())
